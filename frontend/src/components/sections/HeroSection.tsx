...
export default function HeroSection({ lng }: HeroSectionProps) {
  ...
  return (
    <section className="relative py-20 bg-gray-900 overflow-hidden min-h-screen">
      {/* Slider Container */}
      <div
      ...
      >
        {slides.map((slide, index) => (
          <div
            key={index}
            className="w-screen min-h-screen flex-shrink-0"
            style={{
              backgroundImage:
                "linear-gradient(rgba(0, 0, 0, 0.2) 0%, rgba(0, 0, 0, 0.5) 100%), url(" +
                slide +
                ")",
